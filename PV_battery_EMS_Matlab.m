...
input_file = 'Data.xlsx'; N = 48; 

NN = 1;

PD = xlsread(input_file,'','C2:C49')'; % Electrical demand (daily load profile)
PV = xlsread(input_file,'','D2:D49')'; % PV output (daily PV output profile)
RetT = xlsread(input_file,'','E2:E49')'; % Retail electricity tariff
FiT = ones(1,N)*0.08; % Feed-in-tariff
...
% Battery Storage
ebM = 12; % battery maximum storage limit [kWh]
ebm = 2; % battery minimum storage limit [kWh]
pbM = 3; % battery maximum charging rate [kW]
pbm = 3; % battery maximum discharge rate [kW]
